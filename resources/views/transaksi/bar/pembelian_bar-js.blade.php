<script>
    ...
    function getTabelPembelian() {
        $('#dt_pembelian').DataTable({
            ...
            // autoWidth: true,
            ajax: {
                url: "{{ route('getPembelian_bar', ['cabang' => '__cabang__']) }}".replace(
                    '__cabang__', cabang),
                data: function(d) {
    ...
    function getTabelPembelianDetail(id) {
        ...
        var table = $('#dt_pembeliand').DataTable({
            ...
            destroy: true,
            ajax: {
                url: "{{ route('getPembeliand_bar', ['cabang' => '__cabang__']) }}".replace(
                    '__cabang__', cabang),
                data: function(d) {
                    d.transaksi = id
                }
    ...
    $(function() {
        ...
        $("#formInputPembelian").on('submit', function(e) {
            ...
            if ($('#pmbket').val() == '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('addPembelian_bar', ['cabang' => '__cabang__']) }}".replace(
                        '__cabang__', cabang),
                    data: a,
                    processData: false,
                    dataType: "json",
    ...
    $(function() {
        ...
        $("#formInputPembeliand").on('submit', function(e) {
            ...
            if ($('#pmbdparent').val() == '') {
                ...
                $.ajax({
                    type: "POST",
                    url: "{{ route('addPembeliand_bar', ['cabang' => '__cabang__']) }}".replace(
                        '__cabang__', cabang),
                    data: a,
                    processData: false,
                    dataType: "json",
    ...
    function getPickerBahan() {
        ...
        $('#dt_modalBahan2').DataTable({
            ...
            // serverSide: true,
            // autoWidth: true,
            ajax: "{{ route('getAllBahanBar', ['cabang' => '__cabang__']) }}"
                .replace('__cabang__', cabang),
            columns: [{
                    data: 'DT_RowIndex',
                    name: 'DT_RowIndex',
